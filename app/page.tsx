...
import { ThumbsUp, ThumbsDown, Laugh, Send, Trash2, Flame, Clock } from 'lucide-react';
...
function FeedbackContent() {
  ...
  // SHARE STATE (For the hidden card generation)
  ...
  // --- NEW SHARE LOGIC ---
  // 1. Set the item to share (this renders the hidden card)
  const prepareShare = (item: Feedback) => {
    setShareItem(item);
  };

  // 2. Once the hidden card is rendered (via useEffect), snap it
  useEffect(() => {
    if (shareItem) {
      const node = document.getElementById('hidden-share-card');
      if (node) {
        // Give React a split second to render the data into the hidden card
        setTimeout(async () => {
            try {
                const dataUrl = await toPng(node, { 
                    // No complex styling needed here because we styled the hidden card directly
                    cacheBust: true,
                });
  ...
  return (
      <div className="max-w-xl mx-auto min-h-screen relative z-10 px-4 md:px-0">
        ...
        <header className="pt-10 pb-6 flex flex-col items-center justify-center gap-3">
          ...
          <div className="flex bg-white/5 p-1 rounded-full mt-2">
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${filter === 'new' ? 'bg-[#6D4C6F] text-white shadow-lg' : 'text-seismic-muted hover:text-white'}`}
            >
                <Clock className="w-3 h-3" /> New
            </button>
            <button 
                onClick={() => setFilter('hot')}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${filter === 'hot' ? 'bg-[#6D4C6F] text-white shadow-lg' : 'text-seismic-muted hover:text-white'}`}
            >
          ...
          {sortedFeedbacks.map((item) => (
             ...
             >
                ...
                <div className="flex items-center gap-4 text-seismic-muted select-none mt-6">
                  {/* UPVOTE */}
                  <button onClick={() => handleToggle(item.id, 'upvotes')} 
                    className={`flex gap-1.5 transition-all duration-300 items-center px-3 py-1.5 rounded-full border ${item.userVotes.includes('upvote') ? 'border-green-500/50 bg-green-500/10 text-green-400 shadow-[0_0_15px_-4px_rgba(74,222,128,0.5)]' : 'border-transparent hover:bg-white/5 hover:text-green-400'}`}
                  >
                  ...
                  {/* LAUGH (FIXED: NO FILL) */}
                  <button onClick={() => handleToggle(item.id, 'laughs')} 
                    className={`flex gap-1.5 transition-all duration-300 items-center px-3 py-1.5 rounded-full border ${item.userVotes.includes('laugh') ? 'border-yellow-500/50 bg-yellow-500/10 text-yellow-300 shadow-[0_0_15px_-4px_rgba(234,179,8,0.5)]' : 'border-transparent hover:bg-white/5 hover:text-yellow-300'}`}
                  >
                  ...
                  {/* DOWNVOTE */}
                  <button onClick={() => handleToggle(item.id, 'downvotes')} 
                    className={`flex gap-1.5 transition-all duration-300 items-center px-3 py-1.5 rounded-full border ${item.userVotes.includes('downvote') ? 'border-red-500/50 bg-red-500/10 text-red-400 shadow-[0_0_15px_-4px_rgba(248,113,113,0.5)]' : 'border-transparent hover:bg-white/5 hover:text-red-400'}`}
                  >
                  ...
                  {/* SHARE BUTTON (Triggers the hidden card) */}
                  <button 
        ...
        {/* --- HIDDEN SHARE CARD (THE "STUDIO") --- */}
        {/* This is positioned way off screen so the user never sees it, but the camera can snap it. */}
        {shareItem && (
            <div id="hidden-share-card" className="fixed top-0 left-[-9999px] w-[600px] h-[400px] flex items-center justify-center bg-gradient-to-br from-[#0F0514] to-[#3D2242]">
                <div className="w-[500px] bg-[#1A0B1F] p-8 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden">
                    {/* Watermark */}
                    <div className="absolute top-0 right-0 p-4 opacity-20 grayscale">
                         {/* Using text because images might lag in hidden div */}
                         <span className="font-bold text-white text-xs tracking-widest">SEISMIC UNFILTERED</span>
                    </div>
